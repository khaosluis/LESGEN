...
### LIMITATIONS
- Isn't really a limitation, but sometimes, LESGEN[] can generate LES with no single solution. In this case, _eqSol_ just returns a null solution set ({}).
- When all variable coefficients on a LES row are 0, _eqGen_ returns _False_ for that row (no variables!) if and only if the right side of the equation is different from zero. They can also return _True_ if and only if the right side of the equation is zero. It's basically a logic check _(0 equals 3? NOPE, WTF! 0 equals 0? YEAH! of course!)_.
